...
export default function Sidebar({ children }: { children: React.ReactNode; }) {
  ...
  const links = [
    ...
    { name: 'Advisors', to: '/dashboard/advisors', icon: svg.advisors },
    {
      name: 'Report',
      to: '/dashboard/report?language=English',
      icon: svg.report,
    },
  ];
  ...
  return (
    <div className="h-screen flex flex-col">
      ...
      <div className="flex overflow-hidden relative h-full">
        <aside
          className={cn(
            'w-[300px] h-full border-r bg-white border-slate-200 p-5 absolute md:static flex flex-col gap-y-2 transition-all duration-300 ease-in-out overflow-y-auto',
            {
